...
// Theme init
if (!function_exists('axiomthemes_socials_theme_setup')) {
    add_action( 'axiomthemes_action_before_init_theme', 'axiomthemes_socials_theme_setup' );
    function axiomthemes_socials_theme_setup() {
        ...
        if ( is_admin() ) {
            ...
            add_action( 'wp_head', 					'axiomthemes_facebook_og_tags', 5 );
...
$AXIOMTHEMES_GLOBALS['share_links'] = array(
...
if (!function_exists('axiomthemes_add_share_link')) {
    function axiomthemes_add_share_link($soc, $url='') {
        if (!is_array($soc)) $soc = array($soc => $url);
        global $AXIOMTHEMES_GLOBALS;
        $AXIOMTHEMES_GLOBALS['share_links'] = array_merge( $AXIOMTHEMES_GLOBALS['share_links'], $soc );
    }
}

// Return (and show) share social links
if (!function_exists('axiomthemes_show_share_links')) {
    function axiomthemes_show_share_links($args) {
        if ( axiomthemes_get_custom_option('show_share')=='hide' ) return '';
        ...
        $args = array_merge(array(
            ...
            'counters' => axiomthemes_get_custom_option('show_share_counters')=='yes',	// show share counters
            'direction' => axiomthemes_get_custom_option('show_share'),				// share block direction
            'caption' => axiomthemes_get_custom_option('share_caption'),				// share block caption
            'share' => axiomthemes_get_theme_option('share_buttons'),					// list of allowed socials
        ...
        foreach ($args['share'] as $soc) {
            ...
            $sn = $args['style']=='icons' ? axiomthemes_substr($sn, axiomthemes_strpos($sn, '-')+1) : axiomthemes_substr($sn, 0, axiomthemes_strrpos($sn, '.'));

//	        $sn = axiomthemes_substr($sn, 0, axiomthemes_strrpos($sn, '.'));
//	        if (($pos=hotcoffee_strrpos($sn, '_'))!==false)
//		        $sn = hotcoffee_substr($sn, 0, $pos);

	        if (($pos=axiomthemes_strrpos($sn, '_'))!==false)
                $sn = axiomthemes_substr($sn, 0, $pos);

            $url = empty($soc['url']) && !empty($AXIOMTHEMES_GLOBALS['share_links'][$sn]) ? $AXIOMTHEMES_GLOBALS['share_links'][$sn] : $soc['url'];
            ...
            $output .= '<div class="sc_socials_item">'
                ...
                . ($args['type']=='drop' ? '<i>' . trim(axiomthemes_strtoproper($sn)) . '</i>' : '')
                . '</div>';
        }
        $output .= '</div>';
...
if (!function_exists('axiomthemes_prepare_socials')) {
    function axiomthemes_prepare_socials($list, $style='icons') {
        $output = '';
        $upload_info = wp_upload_dir();
        $upload_url = $upload_info['baseurl'];
        if (count($list) > 0) {
            foreach ($list as $soc) {
                ...
                $icon = $style=='icons' || axiomthemes_strpos($soc['icon'], $upload_url)!==false ? $soc['icon'] : axiomthemes_get_socials_url(basename($soc['icon']));
                $sn = basename($soc['icon']);
                if ((axiomthemes_strrpos( $sn, '-' ) > 0 ) or (axiomthemes_strrpos( $sn, '.' ) > 0)) {
                    $sn = $style == 'icons' ? axiomthemes_substr( $sn, axiomthemes_strpos( $sn, '-' ) + 1 ) : axiomthemes_substr( $sn, 0, axiomthemes_strrpos( $sn, '.' ) );
                }
                if (($pos=axiomthemes_strrpos($sn, '_'))!==false)
                    $sn = axiomthemes_substr($sn, 0, $pos);
                $output .= '<div class="sc_socials_item">'
...
if (!function_exists('axiomthemes_show_user_socials')) {
    function axiomthemes_show_user_socials($args) {
        ...
        if (count($list) > 0) {
            $output = '<div class="sc_socials sc_socials_size_'.esc_attr($args['size']).'">' . trim(axiomthemes_prepare_socials($list, $args['style'])) . '</div>';
            if ($args['echo']) echo ($output);
        }
        return $output;
    }
    ...
    function axiomthemes_add_fields_in_user_profile( $user ) {
        ...
        <table class="form-table">
            ...
            foreach ($social_list as $soc) {
                $sn = basename($soc['icon']);
                //$sn = axiomthemes_substr($sn, 0, axiomthemes_strrpos($sn, '-'));
                $sn = axiomthemes_substr( $sn, axiomthemes_strrpos( $sn, '-' ) + 1 );
                if (($pos=axiomthemes_strrpos($sn, '_'))!==false)
                    $sn = axiomthemes_substr($sn, 0, $pos);
                ?>
                <tr>
...
if (!function_exists('axiomthemes_get_twitter_data')) {
    function axiomthemes_get_twitter_data($cfg) {
        $data = get_transient("twitter_data_".($cfg['mode']));
        if (!$data) {
            require_once( axiomthemes_get_file_dir('lib/tmhOAuth/tmhOAuth.php') );
            ...
            $code = $tmhOAuth->user_request(array(
                'url' => $tmhOAuth->url(axiomthemes_get_twitter_mode_url($cfg['mode']))
            ));
            if ($code == 200) {
                $data = json_decode($tmhOAuth->response['response'], true);
                if (isset($data['status'])) {
                    $code = $tmhOAuth->user_request(array(
                        'url' => $tmhOAuth->url(axiomthemes_get_twitter_mode_url($cfg['oembed'])),
                        'params' => array(
        ...
        } else if (!is_array($data) && axiomthemes_substr($data, 0, 2)=='a:') {
...
if (!function_exists('axiomthemes_get_twitter_mode_url')) {
    function axiomthemes_get_twitter_mode_url($mode) {
        $url = '/1.1/statuses/';
...
if (!function_exists('axiomthemes_prepare_twitter_text')) {
    function axiomthemes_prepare_twitter_text($tweet) {
        $text = $tweet['text'];
...
if (!function_exists('axiomthemes_get_twitter_followers')) {
    function axiomthemes_get_twitter_followers($cfg) {
        $data = axiomthemes_get_twitter_data($cfg);
        return $data && isset($data[0]['user']['followers_count']) ? $data[0]['user']['followers_count'] : 0;
    }
    // Old version
    /*
    function axiomthemes_get_twitter_followers($account) {
        $tw = get_transient("twitterfollowers");
        if ($tw !== false) return $tw;
        $tw = '?';
        $url = esc_url('https://twitter.com/users/show/'.($account));
        $headers = get_headers($url);
        if (axiomthemes_strpos($headers[0], '200')) {
            $xml = axiomthemes_fgc($url);
...
if (!function_exists('axiomthemes_get_facebook_likes')) {
    function axiomthemes_get_facebook_likes($account) {
...
function axiomthemes_facebook_og_tags() {
    global $post;
    if ( !is_singular() || axiomthemes_get_global('blog_streampage')) return;
...
if (!function_exists('axiomthemes_get_feedburner_counter')) {
    function axiomthemes_get_feedburner_counter($account) {
