...
// Theme init
if (!function_exists('axiomthemes_less_theme_setup')) {
	//add_action( 'axiomthemes_action_before_init_theme', 'axiomthemes_less_theme_setup' );
	function axiomthemes_less_theme_setup() {
		// Recompile LESS and save CSS
		add_filter("axiomthemes_filter_save_options", 'axiomthemes_less_save_stylesheet', 10, 3);
	}
}

if (!function_exists('axiomthemes_less_theme_setup2')) {
	//add_action( 'axiomthemes_action_after_init_theme', 'axiomthemes_less_theme_setup2' );
	function axiomthemes_less_theme_setup2() {
		// Theme first run - compile and save css
		$theme_data = wp_get_theme();
		$slug = str_replace(' ', '_', trim(axiomthemes_strtolower((string) $theme_data->get('Name'))));
		$option_name = 'axiomthemes_'.strip_tags($slug).'_less_compiled';
		if ( get_option($option_name, false) === false ) {
			add_option($option_name, 1, '', 'yes');
			axiomthemes_less_save_stylesheet(axiomthemes_options_get_all_values(), 'general');
		} else if ( !is_admin() && axiomthemes_get_theme_option('debug_mode')=='yes' ) {
			//axiomthemes_less_save_stylesheet(axiomthemes_options_get_all_values(), 'general');
		}
	}
}
...
if ( !function_exists( 'axiomthemes_less_save_stylesheet' ) ) {
	//add_filter("axiomthemes_filter_save_options", 'axiomthemes_less_save_stylesheet', 10, 3);
	function axiomthemes_less_save_stylesheet($options, $override, $slug) {
		if ($override == 'general') {
			// Compile CSS from LESS files
			axiomthemes_compile_less($options);
		}
		return $options;
	}
}
...
if (!function_exists('axiomthemes_compile_less')) {
	function axiomthemes_compile_less($options) {
	
		// Load and create LESS Parser
		require_once( axiomthemes_get_file_dir('lib/less/Less.php') );
		$parser = new Less_Parser( array( 
			'compress' => axiomthemes_get_theme_option('debug_mode')=='no'
		) );
	
		list($less_vars, $main_styles) = axiomthemes_prepare_less($options);
		
		// Collect .less files in parent and child themes
		$theme_dir = get_template_directory();
		$list = axiomthemes_collect_files($theme_dir, 'less');
		$child_dir = get_stylesheet_directory();
		if ($theme_dir != $child_dir) $list = array_merge($list, axiomthemes_collect_files($child_dir, 'less'));
			...
			foreach($list as $file) {
				...
				try {
					...
					if ($file == $theme_dir . '/style.less') {
						...
						$theme_css = axiomthemes_fgc( get_template_directory() . '/style.css' );
						$css = axiomthemes_substr($theme_css, 0, axiomthemes_strpos($theme_css, '*/')+2) . "\n\n" . $css;
					}
					// Save compiled CSS
					axiomthemes_fpc( substr_replace($file , 'css', strrpos($file , '.') + 1), $css);
				} catch (Exception $e) {
					if (axiomthemes_get_theme_option('debug_mode')=='yes') dfl($e->getMessage());
				}
			}
		}
	}
...
if (!function_exists('axiomthemes_prepare_less')) {
	function axiomthemes_prepare_less($options) {
		$vars = $styles = '';
		/*
		$fonts_list = array( 
			'content', 
...
/*
if (!function_exists('axiomthemes_save_css')) {
	function axiomthemes_save_css($styles, $filename = '') {
