...
// Theme init
if (!function_exists('axiomthemes_po_composer_theme_setup')) {
	add_action( 'axiomthemes_action_after_init_theme', 'axiomthemes_po_composer_theme_setup' );		// Fire this action after load theme options
	function axiomthemes_po_composer_theme_setup() {
		if (is_admin() && current_user_can('manage_options') && axiomthemes_get_theme_option('admin_po_composer')=='yes') {
			new axiomthemes_po_composer();
		}
	}
}


class axiomthemes_po_composer {
	...
	function admin_menu_item() {
		...
		//add_theme_page(__('PO Composer', 'axiomthemes'), __('PO Composer', 'axiomthemes'), 'edit_theme_options', 'po_composer', array($this, 'build_page'));

		// In this case menu item is add in admin menu 'Tools'
		add_management_page(__('PO Composer', 'axiomthemes'), __('PO Composer', 'axiomthemes'), 'manage_options', 'po_composer', array($this, 'build_page'));
	}
	...
	function load_scripts() {
		if (isset($_REQUEST['page']) && $_REQUEST['page']=='po_composer') {
			axiomthemes_enqueue_style('po-composer-style', axiomthemes_get_file_url('tools/po_composer/po_composer.css', array(), null));
		}
		if (isset($_REQUEST['page']) && $_REQUEST['page']=='po_composer') {
			...
			axiomthemes_enqueue_script('po-composer-script', axiomthemes_get_file_url('tools/po_composer/po_composer.js', array('jquery'), null, true));
		}
	}
	...
	function build_page() {
		...
		$options  =  "\n".'<option value="upload_">'.__('Upload .po-file', 'axiomthemes').'</option>'
					."\n".'<option value="edit_"'.(in_array($this->po_src, array('upload_', 'edit_')) ? ' selected="selected"' : '').'>'.__('Edit .po-text', 'axiomthemes').'</option>'
					."\n".'<optgroup label="'.($this->is_child ? __('Parent languages', 'axiomthemes') : __('Languages', 'axiomthemes')).'">'.trim($this->get_list_options($parent_list,  'parent_', $this->po_src))."\n".'</optgroup>'
					.($this->is_child ? "\n".'<optgroup label="'.__('Child languages', 'axiomthemes').'">'.trim($this->get_list_options($child_list, 'child_', $this->po_src))."\n".'</optgroup>' : '');
		$options2 = "\n".'<option value="">'.__('- Select merge source -', 'axiomthemes').'</option>'
					."\n".'<option value="upload_"'.($this->error && $this->po_src2=='upload_' ? ' selected="selected"' : '').'>'.__('Upload .po-file', 'axiomthemes').'</option>'
					."\n".'<option value="edit_"'.($this->error && $this->po_src2=='edit_' ? ' selected="selected"' : '').'>'.__('Edit .po-text', 'axiomthemes').'</option>'
					."\n".'<optgroup label="'.($this->is_child ? __('Parent languages', 'axiomthemes') : __('Languages', 'axiomthemes')).'">'.trim($this->get_list_options($parent_list,  'parent_', $this->error ? $this->po_src2 : ''))."\n".'</optgroup>'
					.($this->is_child ? "\n".'<optgroup label="'.__('Child languages', 'axiomthemes').'">'.trim($this->get_list_options($child_list, 'child_', $this->error ? $this->po_src2 : ''))."\n".'</optgroup>' : '');

		$nonce = wp_create_nonce(__FILE__);
		?>
		<div class="po_composer">
			<h2 class="po_composer_title"><?php _e('Axiomthemes .PO-files Composer', 'axiomthemes'); ?></h2>
			...
			<form id="po_composer_form" action="#" method="post" enctype="multipart/form-data">
				...
				<div class="po_composer_block">
					<fieldset class="po_composer_block_inner">
						<legend><?php _e('Edit .po-file:', 'axiomthemes'); ?></legend>
						<div class="po_composer_fields">
							<div class="po_composer_field">
								<label for="po_src"><?php _e('Select .po-file:', 'axiomthemes'); ?></label>
							...
							<div class="po_composer_field po_composer_editor">
								...
								<input type="checkbox" value="1" name="po_translated_down" id="po_translated_down"<?php echo isset($_POST['po_translated_down']) ? ' checked="checked"' : ''; ?> /><label for="po_translated_down"><?php _e('Move translated strings down', 'axiomthemes'); ?></label>
								</span>
								<ul class="right_side">
									<li id="po_composer_link_text" class="po_composer_link_text"><a href="#po_composer_editor_text"><?php _e('Plain text', 'axiomthemes'); ?></a></li><li id="po_composer_link_list" class="po_composer_link_list"><a href="#po_composer_editor_list"><?php _e('Strings editor', 'axiomthemes'); ?></a></li>
								</ul>
				...
				<div class="po_composer_block">
					<fieldset class="po_composer_block_inner">
						<legend><?php _e('Merge with .po-file:', 'axiomthemes'); ?></legend>
						<div class="po_composer_fields">
							<div class="po_composer_field">
								<label for="po_src2"><?php _e('Select .po-file:', 'axiomthemes'); ?></label>
				...
				<div class="po_composer_buttons">
					<a href="#" id="po_save"><?php echo ($this->po_src=='' ? __('Upload', 'axiomthemes') : __('Update', 'axiomthemes')); ?></a>
					<?php if (!empty($this->po_link) || !empty($this->mo_link)) { ?>
					<div class="po_composer_links">
						<?php if (!empty($this->po_link)) { ?>
							<a href="<?php echo esc_url($this->po_link); ?>" class="po_composer_download_link"><?php _e('Download .PO-file', 'axiomthemes'); ?></a>
						<?php } ?>
						<?php if (!empty($this->mo_link)) { ?>
							<a href="<?php echo esc_url($this->mo_link); ?>" class="po_composer_download_link"><?php _e('Download .MO-file', 'axiomthemes'); ?></a>
						<?php } ?>
					</div>
					<?php } ?>
				</div>
		...
		if (!empty($this->po_src)) {
			if ( !isset($_POST['nonce']) || !wp_verify_nonce( $_POST['nonce'], __FILE__ ) ) {
				...
				do {
					...
					if (($po_data = $this->parse_po($po_text))===false) {
						$this->error = __('Error parsing new .po-file!', 'axiomthemes');
						break;
					}
					if ($this->po_src2 != '') {
						...
						if (($po_data2 = $this->parse_po($po_text2))===false) {
							$this->error = __('Error parsing merging .po-file!', 'axiomthemes');
							break;
						}
					}
			...
			if (empty($this->error)) {
				...
				if (is_writeable($po_temp)) {
					axiomthemes_fpc($po_temp, $this->po_text);
				...
				if (is_writeable($mo_temp)) {
					axiomthemes_fpc($mo_temp, $this->mo_text);
				...
				if (empty($this->po_src2) && !empty($_POST['po_text']) && substr($this->po_src, 0, 7)=='parent_' || substr($this->po_src, 0, 6)=='child_') {
					...
					if (is_writeable($po_file)) {
						axiomthemes_fpc($po_file, $this->po_text);
						$this->success = sprintf(__('Language file "<b>%s.po</b>" was updated!', 'axiomthemes'), $name);
						$mo_file = trim($dir).'/'.trim($name).'.mo';
						if (is_writeable($mo_file)) {
							axiomthemes_fpc($mo_file, $this->mo_text);
							$this->success .= '<br>'.sprintf(__('Language file "<b>%s.mo</b>" was generated!', 'axiomthemes'), $name);
						}
					} else {
						$tjis->error_update = sprintf(__('Error writing in language file "<b>%s.po</b>"!<br>Please, copy text from textarea below and paste it in the .po-file manually<br>or use links to upload .po and .mo files below the text area!', 'axiomthemes'), $name);
					}
				}
			} else {
				$this->po_text = stripslashes($_POST['po_text']);
	...
	function load_po($suffix='') {
		...
		do {
			...
			if ($_POST['po_src'.trim($suffix)] == 'upload_') {
				$rez['data'] = isset($_FILES['po_file'.($suffix)]['tmp_name']) && file_exists($_FILES['po_file'.($suffix)]['tmp_name']) ? axiomthemes_fga($_FILES['po_file'.($suffix)]['tmp_name']) : '';
				if (empty($rez['data'])) {
					$rez['error'] = sprintf(__('Error uploading or Empty .po-file: %s', 'axiomthemes'), $_FILES['po_file'.($suffix)]['tmp_name']);
					break;
				}
			// or get content from textarea
				...
				if (!empty($_POST['po_text'.($suffix)]))
					...
					$rez['error'] = __('Empty textarea with .po-file content!', 'axiomthemes');
					break;
				}
			// or load file from 'languages' folder
				...
				$name = axiomthemes_esc(substr($_POST['po_src'.($suffix)], strpos($_POST['po_src'.($suffix)], '_')+1));
				$rez['data'] = file_exists(($dir).'/'.($name).'.po') ? axiomthemes_fga(($dir).'/'.($name).'.po') : '';
				if (empty($rez['data'])) {
					$rez['error'] = sprintf(__('Error loading or Empty .po-file: %s', 'axiomthemes'), ($dir).'/'.($name).'.po');
					break;
				}
