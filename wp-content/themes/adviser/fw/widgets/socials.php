 ...
 */
add_action( 'widgets_init', 'axiomthemes_widget_socials_load' );

/**
 * Register our widget.
 */
function axiomthemes_widget_socials_load() {
	register_widget( 'axiomthemes_widget_socials' );
}

/**
 * flickr Widget class.
 */
class axiomthemes_widget_socials extends WP_Widget {
	...
	function __construct() {
		/* Widget settings. */
		$widget_ops = array( 'classname' => 'widget_socials', 'description' => __('Show site logo and social links', 'axiomthemes') );

		/* Widget control settings. */
		$control_ops = array( 'width' => 200, 'height' => 250, 'id_base' => 'axiomthemes_widget_socials' );

		/* Create the widget. */
		parent::__construct( 'axiomthemes_widget_socials', __('Axiomthemes - Show logo and social links', 'axiomthemes'), $widget_ops, $control_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		<div class="widget_inner">
            <?php
				if ($show_logo) {
					if ($logo_image=='')	$logo_image = axiomthemes_get_custom_option('logo_image');
					if ($logo_text=='')		$logo_text  = axiomthemes_get_custom_option('logo_text');
	...
	function form( $instance ) {
		...
		$defaults = array( 'title' => '', 'logo_image'=>'', 'logo_text'=>'', 'show_logo' => '1', 'show_icons' => '1', 'text'=>'', 'description' => __('Show logo and social icons', 'axiomthemes') );
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'text' )); ?>"><?php _e('Description:', 'axiomthemes'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'logo_image' )); ?>"><?php _e('Logo image:<br />(if empty - use logo from Theme Options)', 'axiomthemes'); ?></label>
            ...
            <?php
			echo trim(axiomthemes_show_custom_field($this->get_field_id( 'logo_media' ), array('type'=>'mediamanager', 'media_field_id'=>$this->get_field_id( 'logo_image' )), null));
			if ($logo_image) {
			?>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'logo_text' )); ?>"><?php _e('Logo text:', 'axiomthemes'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('show_logo')); ?>_1"><?php _e('Show logo:', 'axiomthemes'); ?></label><br />
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_logo')); ?>_1"><?php _e('Show', 'axiomthemes'); ?></label>
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_logo')); ?>_0"><?php _e('Hide', 'axiomthemes'); ?></label>
		</p>
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('show_icons')); ?>_1"><?php _e('Show social icons:', 'axiomthemes'); ?></label><br />
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_icons')); ?>_1"><?php _e('Show', 'axiomthemes'); ?></label>
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_icons')); ?>_0"><?php _e('Hide', 'axiomthemes'); ?></label>
		</p>
