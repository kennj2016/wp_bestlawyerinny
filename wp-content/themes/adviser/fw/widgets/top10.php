 ...
 */
add_action( 'widgets_init', 'axiomthemes_widget_top10_load' );

/**
 * Register our widget.
 */
function axiomthemes_widget_top10_load() {
	register_widget('axiomthemes_widget_top10');
}

/**
 * Top10 Widget class.
 */
class axiomthemes_widget_top10 extends WP_Widget {
	...
	function __construct() {
		/* Widget settings. */
		$widget_ops = array('classname' => 'widget_top10', 'description' => __('Top 10 posts by average reviews marks (by author and users)', 'axiomthemes'));

		/* Widget control settings. */
		$control_ops = array('width' => 200, 'height' => 250, 'id_base' => 'axiomthemes_widget_top10');

		/* Create the widget. */
		parent::__construct( 'axiomthemes_widget_top10', __('Axiomthemes - Top 10 Posts', 'axiomthemes'), $widget_ops, $control_ops );

		// Add thumb sizes into list
		axiomthemes_add_thumb_sizes( array( 'layout' => 'widgets', 'w' => 75, 'h' => 75, 'h_crop' => 75, 'title'=>__('Widgets', 'axiomthemes') ) );
	}
	...
	function widget($args, $instance) {
		...
		if (count($tabs) > 0) {
			...
			if (count($tabs) > 1) axiomthemes_enqueue_script('jquery-ui-tabs', false, array('jquery','jquery-ui-core'), null, true);
	...
	function form($instance) {
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('title_author')); ?>"><?php _e('Author rating tab title:', 'axiomthemes'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('title_users')); ?>"><?php _e('Users rating tab title:', 'axiomthemes'); ?></label>
		...
		<p>
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_counters')); ?>_2"><?php _e('As stars', 'axiomthemes'); ?></label>
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_counters')); ?>_1"><?php _e('As icon', 'axiomthemes'); ?></label>
