...
function axiomthemes_skin_customizer(option, val) {
	var custom_style = '';
	if (option == 'bg_color') {
		...
		var clr = val;
		var rgb = axiomthemes_hex2rgb(clr);
		// Link color styles
		custom_style += '\
			a,\
			.bg_tint_light a,\
	...
	//} else if (option == 'body_style') {
		...
		var params = AXIOMTHEMES_GLOBALS['co_add_params']!=undefined ? AXIOMTHEMES_GLOBALS['co_add_params'] : {};
		params[option] = val;
	...
	if (custom_style != '') {
		var styles = jQuery('#axiomthemes-customizer-styles-'+option).length > 0 ? jQuery('#axiomthemes-customizer-styles-'+option) : '';
		if (styles.length == 0)
			jQuery('head').append('<style id="axiomthemes-customizer-styles-'+option+'" type="text/css">'+custom_style+'</style>');
		else
