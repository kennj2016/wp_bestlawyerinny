...
if (!$post_data['post_protected']) {
	...
	if (!empty($post_options['dedicated'])) {
		...
		$page_style = axiomthemes_get_custom_option('single_style');
		?>
		<div class="post_thumb post_nav"<?php echo ($page_style=='single-portfolio-fullscreen' ? ' style="background-image:url('.esc_url($post_data['post_attachment']).');"' : ''); ?>>
			...
			$args = axiomthemes_query_add_posts_and_cats($args, '', $post_data['post_type'], !empty($post_data['post_terms'][$post_data['post_taxonomy']]->terms_ids) ? join(',', $post_data['post_terms'][$post_data['post_taxonomy']]->terms_ids : ''), $post_data['post_taxonomy']);
			...
			if ( $prev ) {
				...
				$desc = axiomthemes_strshort($prev->post_title, 30);
				?>
			...
			if ( $next ) {
				...
				$desc = axiomthemes_strshort($next->post_title, 30);
				?>
