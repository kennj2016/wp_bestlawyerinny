<?php
//===================================== Post author info =====================================
if (axiomthemes_get_custom_option("show_post_author") == 'yes') {
	$post_author_name = $post_author_socials = '';
	$show_post_author_socials = false;
	if ($post_data['post_type']=='post') {
		$post_author_title = __(' ', 'axiomthemes');
		...
		$post_author_avatar = get_avatar($post_author_email, 75*min(2, max(1, axiomthemes_get_theme_option("retina_ready"))));
		$post_author_descr = do_shortcode(nl2br(get_the_author_meta('description', $post_data['post_author_id'])));
		if ($show_post_author_socials) $post_author_socials = axiomthemes_show_user_socials(array('author_id'=>$post_data['post_author_id'], 'style'=>'bg', 'size'=>'tiny', 'echo' => false));
	} else if ($post_data['post_type']=='courses') {
		$post_author_id = axiomthemes_get_custom_option('teacher');
		if ($post_author_id) {
			global $post;
			$post = get_post($post_author_id);
			setup_postdata($post);
			$post_author_title = __('About Teacher', 'axiomthemes');
			...
			if (empty($post_author_avatar)) {
				...
				$post_author_avatar = axiomthemes_get_resized_image_tag($post_author_avatar, 75, 75);
			}
