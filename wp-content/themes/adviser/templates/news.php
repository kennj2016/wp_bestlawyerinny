...
if ( !function_exists( 'axiomthemes_template_news_theme_setup' ) ) {
	add_action( 'axiomthemes_action_before_init_theme', 'axiomthemes_template_news_theme_setup', 1 );
	function axiomthemes_template_news_theme_setup() {
		axiomthemes_add_template(array(
			'layout' => 'news',
			'mode'   => 'blogger',
			'need_columns' => true,
			'title'  => __('Blogger layout: News', 'axiomthemes'),
...
// Template output
if ( !function_exists( 'axiomthemes_template_news_output' ) ) {
	function axiomthemes_template_news_output($post_options, $post_data) {
		...
		if (axiomthemes_sc_param_is_on($post_options['scroll']) || ($post_options['dir'] == 'horizontal' && $post_options['columns_count'] > 0)) {
			?>
		...
		<div class="post_item post_item_news sc_blogger_item<?php echo ($post_options['number'] == $post_options['posts_on_page'] && !axiomthemes_sc_param_is_on($post_options['loadmore']) ? ' sc_blogger_item_last' : '');
			//. (axiomthemes_sc_param_is_on($post_options['scroll']) && $post_options['dir'] == 'vertical' ? ' sc_scroll_slide swiper-slide' : '');
			?>">
			<?php 
